...
function changeFieldRequestHandler(event) {
    ...
    if (excursionDate.value != '' && startTime.value != '') {
        ...
        if (option1.checked) calculateTotalCost *= 0.75;
...
async function controlItemsHandler(event) {
    ...
    if (action) {
        ...
        if (action == 'show') {
            ...
            nameOption1.innerHTML = 'скидка для пенсионеров';
            nameOption2.innerHTML = 'трансфер после экскурсии';
            descOption1.innerHTML = 'скидка для пенсионеров стоимость \
            уменьшается на ';
